...
public class RecruitingControllerTest {
    ...
    @Test
    public void getJobList_Ok() throws Exception{
        ...
        List<Job> jobs = List.of(jbo_result_1, jbo_result_2);

        Optional<List<Job>> results = Optional.of(jobs);

        Mockito.when(jobService.getJobList()).thenReturn(results);
    ...
    @Test
    public void getJobListEmpty_Ok() throws Exception{
        String url = "/api/v1/jobs";

        List<Job> jobsEmpty = Collections.emptyList();

        Optional<List<Job>> resultsEmpty = Optional.of(jobsEmpty);

        Mockito.when(jobService.getJobList()).thenReturn(resultsEmpty);
    ...
    @Test
    public void getJobList_InternalServerError() throws Exception{
        String url = "/api/v1/jobs";

        Mockito.when(jobService.getJobList()).thenThrow(HttpServerErrorException.InternalServerError.class);
    ...
    @Test
    public void postJobApplication_Ok() throws Exception{
        ...
        Optional<ResponseApplyForJob> results = Optional.of(responseApplyForJob);

        Mockito.when(jobService.applyForJobLead(ArgumentMatchers.any(ApplyForJobLead.class))).thenReturn(results);

        JSONObject applyForJobLead = new JSONObject();

        applyForJobLead.put("firstName", "Testfirstname_DONT-USE");
        applyForJobLead.put("lastName", "Testlastname_DONT-USE");
        applyForJobLead.put("email", "cr.careers.by.pdg@yopmail.com");
        applyForJobLead.put("phoneNumber", "50683288274");
        applyForJobLead.put("country", "Costa Rica");
        applyForJobLead.put("jobId", "743999812777904-senior-creative-engineer---e-mail");
        applyForJobLead.put("srUuid", "905f658c-bef0-4597-b1ff-b5e69c962edb");
        applyForJobLead.put("linkedinUrl", "linedInd/test_DONT-USE");

        var jsonPost = applyForJobLead.toString();
    ...
    @Test
    public void postJobApplyment_InternalServerError() throws Exception{
        String url = "/api/v1/jobs-application";

        Mockito.when(jobService.applyForJobLead(ArgumentMatchers.any(ApplyForJobLead.class))).thenThrow(HttpServerErrorException.InternalServerError.class);
